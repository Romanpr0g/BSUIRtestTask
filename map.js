...
function ensureSvgGlowDefs(svg, okHex, badHex) {
  ...
  const makeBoth = (id, color) => {
    ...
    // --- Внешнее свечение ---
    const oBlur = el("feGaussianBlur", {
      in: "SourceAlpha",
      stdDeviation: "3",
      result: "o-blur",
    });
    const oFlood = el("feFlood", {
      "flood-color": color,
      "flood-opacity": "1",
      result: "o-col",
    });
    const oComp = el("feComposite", {
      in: "o-col",
      in2: "o-blur",
      operator: "in",
      result: "outerGlow",
    ...
    const iCore = el("feMorphology", {
      in: "SourceAlpha",
      operator: "erode",
      radius: "1.6",
      result: "i-core",
    });
    const iRing = el("feComposite", {
      in: "SourceAlpha",
      in2: "i-core",
      operator: "out",
      result: "i-ring",
    });
    const iBlur = el("feGaussianBlur", {
      in: "i-ring",
      stdDeviation: "2.5",
      result: "i-blur",
    });
    const iFlood = el("feFlood", {
      "flood-color": color,
      "flood-opacity": "1",
      result: "i-col",
    });
    const iComp = el("feComposite", {
      in: "i-col",
      in2: "i-blur",
      operator: "in",
      result: "innerGlow",
    ...
    (e) => {
      ...
      if (!down.moved) {
        const dx = e.clientX - down.x,
          dy = e.clientY - down.y;
        if (Math.abs(dx) > CLICK_TOL || Math.abs(dy) > CLICK_TOL)
          down.moved = true;
      }
    },
    { passive: true }
    ...
    (e) => {
      ...
      if (e.button === 0 && !down.moved) {
        ...
        const isEmpty =
          ...
          t === pzInner ||
          (t.classList &&
            (t.classList.contains("map-stage") ||
              t.classList.contains("backgrid") ||
              t.classList.contains("map-svg"))) ||
...
function applyJsonDataClient(d, { setImage = true } = {}) {
  ...
  [...mapStage.querySelectorAll(".area, svg.map-svg")].forEach((n) =>
    n.remove()
  );
  selected.clear();
  ...
  const hasPolys = arr.some(
    (o) => Array.isArray(o.coords) && o.coords.length >= 3
  );
  const newAreas = [];
