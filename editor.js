...
function segIntersects(a1, a2, b1, b2) {
  const o1 = orient(a1, a2, b1),
    ...
    o3 = orient(b1, b2, a1),
    o4 = orient(b1, b2, a2);
  if (Math.abs(o1) < EPS && onSeg(a1, a2, b1)) return true;
  if (Math.abs(o2) < EPS && onSeg(a1, a2, b2)) return true;
  if (Math.abs(o3) < EPS && onSeg(b1, b2, a1)) return true;
  if (Math.abs(o4) < EPS && onSeg(b1, b2, a2)) return true;
...
function segmentIntersectionPoint(a1, a2, b1, b2) {
  ...
  const pt = [xi, yi];
  return onSeg(a1, a2, pt) && onSeg(b1, b2, pt) ? { x: xi, y: yi } : null;
}
...
function hasCollision(area) {
  for (const o of areas) {
    if (
      o.id !== area.id &&
      !selectedSet.has(o.id) &&
      polysIntersect(area.points, o.points)
    )
      return true;
  }
  return false;
...
function anyCollisionForSnapshot(snapshot, dx, dy) {
  for (const [sid, pts] of snapshot.entries()) {
    ...
    for (const o of areas) {
      if (snapshot.has(o.id)) continue;
      if (polysIntersect(moved, o.points)) return true;
    }
  }
  return false;
}
function firstIntersectionAlongSegment(p1, p2) {
  let best = null;
  for (const o of areas) {
    for (let i = 0; i < o.points.length; i++) {
      const ip = segmentIntersectionPoint(
        [p1[0], p1[1]],
        [p2[0], p2[1]],
        [o.points[i][0], o.points[i][1]],
        [
          o.points[(i + 1) % o.points.length][0],
          o.points[(i + 1) % o.points.length][1],
        ]
      );
      if (ip) {
        const d = Math.hypot(ip.x - p1[0], ip.y - p1[1]);
        if (!best || d < best.d) best = { pt: ip, polyId: o.id, dist: d };
      }
    }
  }
  return best;
...
function px2r(px) {
  ...
  const k = overlay.viewBox.baseVal.width / Math.max(1, rect.width);
  return px * k;
}
function toSvg(cx, cy) {
  const rect = overlay.getBoundingClientRect();
  const x =
    ((cx - rect.left) / Math.max(1, rect.width)) *
    overlay.viewBox.baseVal.width;
  const y =
    ((cy - rect.top) / Math.max(1, rect.height)) *
    overlay.viewBox.baseVal.height;
  return { x: Math.max(0, Math.min(x, iw)), y: Math.max(0, Math.min(y, ih)) };
}
function centerLayers() {
  const innerW = parseFloat(panInner.style.width) || panInner.clientWidth;
  const innerH = parseFloat(panInner.style.height) || panInner.clientHeight;
  const left = (innerW - iw) / 2;
  const top = (innerH - ih) / 2;
...
function ensureRootLayers() {
  if (!overlay.querySelector("rect[data-bg]")) {
    const bg = document.createElementNS("http://www.w3.org/2000/svg", "rect");
    bg.dataset.bg = "1";
    bg.setAttribute("fill", "transparent");
    bg.style.pointerEvents = "fill";
    overlay.appendChild(bg);
  }
  if (!overlay.querySelector("#areas")) {
    const g = document.createElementNS("http://www.w3.org/2000/svg", "g");
    g.id = "areas";
    overlay.appendChild(g);
  }
  if (!overlay.querySelector("#handles")) {
    const g = document.createElementNS("http://www.w3.org/2000/svg", "g");
    g.id = "handles";
    overlay.appendChild(g);
  }
  if (!overlay.querySelector("#draft")) {
    const g = document.createElementNS("http://www.w3.org/2000/svg", "g");
    g.id = "draft";
    overlay.appendChild(g);
  }
  if (!overlay.querySelector("#ghosts")) {
    const g = document.createElementNS("http://www.w3.org/2000/svg", "g");
    g.id = "ghosts";
    overlay.appendChild(g);
  }
  ...
  bg.setAttribute("x", 0);
  bg.setAttribute("y", 0);
  bg.setAttribute("width", iw);
  bg.setAttribute("height", ih);
}
function updateStageFromImage(prevMeta) {
  ...
  ensureRootLayers();
  centerLayers();
  if (prevMeta && prevMeta.iw && (prevMeta.iw !== iw || prevMeta.ih !== ih)) {
    const sx = iw / prevMeta.iw,
      sy = ih / prevMeta.ih;
...
function saveCacheFast() {
  ...
  localStorage.setItem(CACHE_KEY_FAST, JSON.stringify(payload));
}
  ...
  try {
    ...
    if (Array.isArray(d.areas)) {
      areas = d.areas.map((o) => ({
        ...
        points: o.coords.map((p) => [+p[0], +p[1]]),
        colorName: o.color,
      }));
      nextId =
        d.nextId || areas.reduce((m, a) => Math.max(m, a.id), 0) + 1 || 1;
    }
    return d;
  } catch {
    return null;
...
function clearTransient() {
  overlay.querySelector("#handles").innerHTML = "";
  overlay.querySelector("#draft").innerHTML = "";
  overlay.querySelector("#ghosts").innerHTML = "";
  removePhantom();
  hideEdgeGhost();
  curCountEl.textContent = "0";
}
function clearSelection() {
  selectedSet.clear();
  curIdEl.textContent = "нет";
  clearTransient();
  renderAll();
  renderList();
}
...
//======== rAF рескейл ===========
let rafRescale = 0;
function requestRescale() {
  if (rafRescale) return;
  rafRescale = requestAnimationFrame(() => {
    rafRescale = 0;
    rescaleOverlay();
  });
}
...
function rescaleOverlay() {
  const setR = (sel, pxR, sw = 0.9) =>
    overlay.querySelectorAll(sel).forEach((c) => {
      c.setAttribute("r", px2r(pxR));
      c.setAttribute("stroke-width", px2r(sw));
    });
  ...
  if (start) {
    start.setAttribute(
      "r",
      px2r(draftPts.length >= 3 ? DOT_PX.startBig : DOT_PX.start)
    );
    start.setAttribute("stroke-width", px2r(0.9));
  }
  const ch = overlay.querySelector(".center-handle");
  if (ch) {
    ch.setAttribute("r", px2r(DOT_PX.center));
    ch.setAttribute("stroke-width", px2r(1));
  }
  const eg = edgeGhost;
  if (eg) {
    eg.setAttribute("r", px2r(DOT_PX.ghost));
    eg.setAttribute("stroke-width", px2r(1));
  }

  overlay
    .querySelectorAll(".draft-line")
    .forEach((l) => l.setAttribute("stroke-width", px2r(LINE_PX.draft)));
  overlay
    .querySelectorAll("#draft line")
    .forEach((l) => l.setAttribute("stroke-width", px2r(LINE_PX.phantom)));
}
...
function renderAll() {
  ...
  for (const a of areas) {
    ...
    updatePoly(a);
    const hex = (palette.find((c) => c.name === a.colorName) || { hex: "#fff" })
      .hex;
...
function showHandlesForSelection() {
  if (isDrawing) return;
  const g = overlay.querySelector("#handles");
  g.innerHTML = "";
  if (selectedSet.size === 0) return;

  if (selectedSet.size === 1) {
    const sid = [...selectedSet][0];
    const area = areas.find((x) => x.id === sid);
    area.points.forEach((p, idx) => {
      ...
      c.setAttribute("r", px2r(DOT_PX.handle));
      ...
      c.addEventListener("pointerdown", (ev) => {
        ...
        panInner.classList.add("cursor-move");
        c.setPointerCapture(ev.pointerId);
      });
      c.addEventListener("pointermove", (ev) => {
        if (!dragging) return;
        const cur = toSvg(ev.clientX, ev.clientY),
          prev = toSvg(last.x, last.y);
        let dx = cur.x - prev.x,
          dy = cur.y - prev.y;
        if (ev.shiftKey) {
          dx *= 0.1;
        ...
        const old = [area.points[idx][0], area.points[idx][1]];
        area.points[idx][0] = Math.max(0, Math.min(old[0] + dx, vb.width));
        area.points[idx][1] = Math.max(0, Math.min(old[1] + dy, vb.height));
        if (hasCollision(area)) {
          area.points[idx][0] = old[0];
          area.points[idx][1] = old[1];
          flashRed(c);
        } else {
          updatePoly(area);
          c.setAttribute("cx", area.points[idx][0]);
          c.setAttribute("cy", area.points[idx][1]);
        }
        last = { x: ev.clientX, y: ev.clientY };
      });
      c.addEventListener("pointerup", (ev) => {
        dragging = false;
        panInner.classList.remove("cursor-move");
        try {
  ...
  const selectedAreas = [...selectedSet].map((id) =>
  ...
  if (selectedAreas.length) {
    ...
    cc.classList.add("center-handle");
    cc.setAttribute("cx", cx);
    cc.setAttribute("cy", cy);
    cc.setAttribute("r", px2r(DOT_PX.center));
    let dragAll = false,
      origin = { x: 0, y: 0 },
      snapshot = null;
    cc.addEventListener("pointerdown", (ev) => {
      ...
      snapshot = new Map(
        selectedAreas.map((a) => [a.id, a.points.map((p) => [p[0], p[1]])])
      );
      panInner.classList.add("cursor-move");
    ...
    cc.addEventListener("pointermove", (ev) => {
      ...
      for (const [sid, pts] of snapshot.entries()) {
        ...
        for (let i = 0; i < pts.length; i++) {
          a.points[i][0] = pts[i][0] + dx;
          a.points[i][1] = pts[i][1] + dy;
        }
        updatePoly(a);
      }
      if (selectedSet.size === 1) {
    ...
    cc.addEventListener("pointerup", (ev) => {
      dragAll = false;
      panInner.classList.remove("cursor-move");
      try {
        cc.releasePointerCapture(ev.pointerId);
      } catch {}
      saveCacheDebounced();
      renderAll();
    });
...
function renderDraft() {
  const g = overlay.querySelector("#draft");
  g.innerHTML = "";
  ...
  draftPts.forEach((p, i) => {
    ...
    if (i === 0) {
      ...
      c.addEventListener("pointerdown", (ev) => {
        if (ev.button !== 0) return;
        ev.stopPropagation();
        fCand = { x: ev.clientX, y: ev.clientY, moved: false };
        c.setPointerCapture?.(ev.pointerId);
      });
      c.addEventListener("pointermove", (ev) => {
        if (!fCand) return;
        if (
          !fCand.moved &&
          (Math.abs(ev.clientX - fCand.x) > 6 ||
            Math.abs(ev.clientY - fCand.y) > 6)
        )
          fCand.moved = true;
      });
      c.addEventListener("pointerup", (ev) => {
        if (!fCand) return;
        if (!fCand.moved && draftPts.length >= 3) finalizeDraft();
        try {
          c.releasePointerCapture?.(ev.pointerId);
        } catch {}
        fCand = null;
...
function ensurePhantom() {
  ...
  if (!phantom) {
    phantom = document.createElementNS("http://www.w3.org/2000/svg", "circle");
    phantom.setAttribute("class", "draft-dot");
    phantom.setAttribute("fill", "rgba(255,0,0,0.85)");
    phantom.style.pointerEvents = "none";
    d.appendChild(phantom);
  }
  if (!phantomLine) {
    phantomLine = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "line"
    );
    phantomLine.setAttribute("class", "ghost-line");
    phantomLine.style.pointerEvents = "none";
    d.appendChild(phantomLine);
  }
  if (!phantomLineStart) {
    phantomLineStart = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "line"
    );
    phantomLineStart.setAttribute("class", "ghost-line");
    phantomLineStart.style.pointerEvents = "none";
    d.appendChild(phantomLineStart);
  }
}
...
function finalizeDraft() {
  ...
  const newA = {
    id: nextId++,
    name: `Область ${nextId - 1}`,
    points: draftPts.slice(),
    colorName: selectedColor.name,
  };
  areas.push(newA);
  if (hasCollision(newA)) {
    for (const o of areas) {
      if (o.id !== newA.id && polysIntersect(newA.points, o.points)) {
        const els = areaEls.get(o.id);
        flashRed(els?.poly);
      }
    }
    areas = areas.filter((a) => a.id !== newA.id);
  } else {
    saveCacheDebounced();
  }
  cancelDrawing();
...
function addDraftPtFromMouse(cx, cy) {
  ...
  if (draftPts.length) {
    ...
    if (inter) {
      const vx = inter.pt.x - last[0],
        vy = inter.pt.y - last[1];
      const len = Math.hypot(vx, vy) || 1;
      const safe = Math.max(0, len - 1);
      draftPts.push([
        Math.round(last[0] + (vx / len) * safe),
        Math.round(last[1] + (vy / len) * safe),
      ]);
      const els = areaEls.get(inter.polyId);
      flashRed(els?.poly);
    } else {
  ...
  curCountEl.textContent = String(draftPts.length);
  renderDraft();
}

//======== Вставка/удаление вершин ===========
function nearestEdgeIndex(points, x, y) {
  let best = { idx: -1, dist: Infinity, proj: [x, y] };
  for (let i = 0; i < points.length; i++) {
    ...
    if (d < best.dist) best = { idx: i, dist: d, proj: [px, py] };
  }
  return best;
}
function insertVertexAtPoint(a, proj) {
  const e = nearestEdgeIndex(a.points, proj[0], proj[1]);
  const insertIdx = (e.idx + 1) % a.points.length;
  a.points.splice(insertIdx, 0, [Math.round(proj[0]), Math.round(proj[1])]);
  updatePoly(a);
  renderAll();
  renderList();
  saveCacheDebounced();
}
function deleteVertexAtHandle(a, idx) {
  if (a.points.length <= 3) return;
  a.points.splice(idx, 1);
  updatePoly(a);
  renderAll();
  renderList();
  saveCacheDebounced();
...
function ensureEdgeGhost() {
  ...
  if (!edgeGhost) {
    edgeGhost = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "circle"
    );
    edgeGhost.setAttribute("class", "ghost-dot");
    g.appendChild(edgeGhost);
  }
}
function hideEdgeGhost() {
  overlay.querySelector("#ghosts").innerHTML = "";
  edgeGhost = null;
}
function showEdgeGhost(px, py) {
  ensureEdgeGhost();
  edgeGhost.setAttribute("cx", px);
  edgeGhost.setAttribute("cy", py);
  requestRescale();
}
...
overlay.addEventListener("pointermove", (ev) => {
  ...
  if (isDrawing) {
    ...
    if (draftPts.length) {
      ...
      if (inter) {
        ...
        const len = Math.hypot(vx, vy) || 1;
        const safe = Math.max(0, len - 1);
        tx = last[0] + (vx / len) * safe;
        ty = last[1] + (vy / len) * safe;
      }
      phantom.setAttribute("cx", tx);
      phantom.setAttribute("cy", ty);
      phantomLine.setAttribute("x1", last[0]);
      phantomLine.setAttribute("y1", last[1]);
      phantomLine.setAttribute("x2", tx);
      phantomLine.setAttribute("y2", ty);
      const s = draftPts[0];
      if (s) {
        phantomLineStart.setAttribute("x1", s[0]);
        phantomLineStart.setAttribute("y1", s[1]);
        phantomLineStart.setAttribute("x2", tx);
        phantomLineStart.setAttribute("y2", ty);
      }
    } else {
      phantom.setAttribute("cx", mp.x);
      phantom.setAttribute("cy", mp.y);
      phantomLine.setAttribute("x1", mp.x);
      phantomLine.setAttribute("y1", mp.y);
      phantomLine.setAttribute("x2", mp.x);
      phantomLine.setAttribute("y2", mp.y);
      phantomLineStart.setAttribute("x1", mp.x);
      phantomLineStart.setAttribute("y1", mp.y);
      phantomLineStart.setAttribute("x2", mp.x);
      phantomLineStart.setAttribute("y2", mp.y);
    }
    return;
  }
  ...
  if (selectedSet.size === 1) {
    ...
    const nearVertex = a.points.some(
      (pt) => Math.hypot(pt[0] - mp.x, pt[1] - mp.y) <= tolHandle
    );
    const bb = a.points.reduce(
      (acc, p) => ({
        minX: Math.min(acc.minX, p[0]),
        maxX: Math.max(acc.maxX, p[0]),
        minY: Math.min(acc.minY, p[1]),
    ...
    const cx = (bb.minX + bb.maxX) / 2,
      cy = (bb.minY + bb.maxY) / 2;
    const nearCenter =
      Math.hypot(mp.x - cx, mp.y - cy) <= px2r(DOT_PX.center * 1.2);
...
overlay.addEventListener("pointerdown", (ev) => {
  if (isDrawing && ev.button === 0)
    clickCand = { x: ev.clientX, y: ev.clientY, moved: false };
});
window.addEventListener("pointermove", (ev) => {
  ...
  if (!clickCand.moved) {
    ...
    if (Math.abs(dx) > CLICK_TOL_PX || Math.abs(dy) > CLICK_TOL_PX)
      clickCand.moved = true;
  }
});
window.addEventListener("pointerup", (ev) => {
  if (!clickCand) return;
  if (isDrawing && ev.button === 0 && !clickCand.moved)
    addDraftPtFromMouse(ev.clientX, ev.clientY);
  clickCand = null;
});
...
overlay.addEventListener("pointerdown", (ev) => {
  ...
  if (
    idx >= 0 &&
    Math.hypot(a.points[idx][0] - p.x, a.points[idx][1] - p.y) <= tolHandle &&
    a.points.length > 3
  ) {
    deleteVertexAtHandle(a, idx);
    saveCacheDebounced();
    return;
  }
...
function bindBgClearSelection() {
  ...
  window.addEventListener("pointermove", (ev) => {
    ...
    if (
      !bgClick.moved &&
      (Math.abs(ev.clientX - bgClick.x) > 6 ||
        Math.abs(ev.clientY - bgClick.y) > 6)
    )
      bgClick.moved = true;
  });
  bg.addEventListener("pointerup", (ev) => {
    if (!bgClick) return;
    if (!isDrawing && ev.button === 0 && !bgClick.moved) {
      clearSelection();
    }
    bgClick = null;
  });
...
window.addEventListener("keydown", (ev) => {
  ...
  if (ev.key === "Control" && !ev.repeat && !isDrawing) {
    drawingByCtrl = true;
    startDrawing();
  }
  if (isDrawing && ev.key === "Escape") {
    cancelDrawing();
    drawingByCtrl = false;
  }
  if (isDrawing && ev.key === "Enter") {
    finalizeDraft();
    drawingByCtrl = false;
  }
  if (!isDrawing && key === "delete") {
    if (selectedSet.size) {
  ...
  if (!isDrawing && ev.key === "+") {
    if (selectedSet.size > 1) mergeSelectedAsHull();
  }
});
window.addEventListener("keyup", (ev) => {
  if (ev.key === "Control" && drawingByCtrl) {
    if (isDrawing) {
      if (draftPts.length >= 3) finalizeDraft();
      else cancelDrawing();
    }
    drawingByCtrl = false;
  }
...
function renderControls() {
  ...
  const btn = (id, text, kind = "primary", disabled = false) =>
    `<button id="${id}" class="${kind === "secondary" ? "secondary" : ""} ${
      kind === "danger" ? "danger" : ""
  ...
  byId("btnToggleDraw")?.addEventListener("click", () =>
    isDrawing ? finalizeDraft() : startDrawing()
  );
    ...
    inp.addEventListener("change", (e) => {
      ...
      fr.onload = () => {
        try {
          ...
          const prevMeta =
            d.meta && (d.meta.iw || d.meta.ih)
              ? { iw: d.meta.iw, ih: d.meta.ih }
              : null;
...
function renderList() {
  areasList.innerHTML = "";
  if (areas.length === 0) {
    areasList.textContent = "Нет областей";
    return;
  }
  areas
    .slice()
    ...
    .forEach((a) => {
      ...
      div.innerHTML = `<div class="row"><input class="name-input" value="${
        a.name || "Область " + a.id
      }" data-id="${a.id}" title="Название области"/></div>
      <div class="row"><div class="mini" style="background:${
        (palette.find((c) => c.name === a.colorName) || { hex: "#fff" }).hex
      }"></div>
      <button class="secondary" data-id="${
        a.id
      }" data-act="sel">Выбрать</button>
      <button class="danger" data-id="${
        a.id
      }" data-act="del">Удалить</button></div>`;
      areasList.appendChild(div);
    });
  areasList.querySelectorAll("button").forEach((b) =>
  ...
  areasList.querySelectorAll(".name-input").forEach((inp) =>
    inp.addEventListener("change", (e) => {
      const id = +e.currentTarget.dataset.id;
      const a = areas.find((x) => x.id === id);
      if (a) {
        a.name = e.currentTarget.value.trim() || `Область ${a.id}`;
        saveCacheDebounced();
      }
    })
  );
...
function mergeSelectedAsHull() {
  ...
  const pts = [...selectedSet].flatMap(
    (id) => areas.find((a) => a.id === id)?.points || []
  );
  if (pts.length < 3) return;
  const hull = convexHull(pts);
  const color =
    areas.find((a) => selectedSet.has(a.id))?.colorName || selectedColor.name;
  ...
  for (const o of others) {
    if (polysIntersect(hull, o.points)) {
      const els = areaEls.get(o.id);
      flashRed(els?.poly);
      return;
    }
...
function init() {
  ...
  const prevMeta = cached?.meta
    ? { iw: cached.meta.iw, ih: cached.meta.ih }
    : null;
  img.onload = () => {
    updateStageFromImage(prevMeta);
    renderList();
